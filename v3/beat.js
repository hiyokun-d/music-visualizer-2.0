...
function makeAbeat() {
    ...
    // if (beatDetected) {
    //     if (isFirstBeat) {
    //         // Play initial beat animation using direct CSS animation
    //         videoPlayer.classList.add("scale")
    //         setTimeout(() => {
    //             isFirstBeat = false;
    //         }, 500);
    //     } else {
    //         // Apply CSS animation for subsequent beats
    //         videoPlayer.style.animation = "beat 1s infinite ease"
    //     }
    // } else {
    //     // No beat detected, revert to normal state
    //     if (!isFirstBeat) {
    //         isFirstBeat = false;
    //         videoPlayer.style.animation = "shake 1s infinite ease-in-out";
    //     }
    // }
    ...
    if (beatDetected) {
        if (isFirstBeat) {
            gsap.to(videoPlayer, {
                ...
                // ease: "power4.out",
                delay: 0,
                onComplete: () => gsap.to(videoPlayer, {
